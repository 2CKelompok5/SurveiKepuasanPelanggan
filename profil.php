...
switch ($kategori_user_id) {
    ...
    case 5:
        ...
        $fields = ['responden_nim', 'responden_nama', 'responden_email', 'responden_hp', 'tahun_lulus', 'responden_prodi', 'jurusan'];
        break;
    case 6:
...
$sql = "SELECT $field_list FROM $table WHERE user_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $columns_str = implode(',', array_merge(['user_id'], $fields));
    $placeholders_str = implode(',', array_fill(0, count($fields) + 1, '?'));
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param(str_repeat('s', count($post_values) + 1), $user_id, ...array_values($post_values));
...
<head>
    ...
    <style>
        ...
        .container {    
            margin-top: 20px;padding-right: 90px;
        }
        .profile-form {
        ...
        .profile-form .form-control {
            margin-bottom: 15px;padding-right: 90px;
        }
        .profile-form button {
            background-color: #007bff;
            color: #fff;padding-right: 90px;
        }
    </style>
</head>
<body>
